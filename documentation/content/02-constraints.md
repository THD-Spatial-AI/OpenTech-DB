...
| TC-04 | Data storage: backend-only **Supabase PostgreSQL/PostgREST**, with JSON seeds/fallback | Supports catalogue and workflow queries while retaining portable, version-controlled seed data. |
...
| TC-10 | Frontend auth: same-origin **Go auth service** backed by the `opentechdb` Keycloak realm | The browser receives only opaque HttpOnly sessions; Supabase is data-only. |
